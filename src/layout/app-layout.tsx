...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['clinic-owner', 'patint', 'admin'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<DoctorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/doctors` : null),
    () => getDoctors({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Doctor',
      path: isTenantUser ? `/doctors/view/${data?.[0]?.id}` : '/doctors',
      entity: 'doctor',
    ...
    { name: 'Patient', path: '/patients', entity: 'patient', service: AccessServiceEnum.PROJECT, icon: FiUserPlus },
