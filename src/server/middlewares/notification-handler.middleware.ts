...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'doctor.create': { roles: ['admin'], key: 'doctor-profile-created', tenantPath: ['doctor'], userPath: [] },
  'appointment.create': {
    roles: ['clinic-owner', 'admin'],
    key: 'appointment-booked',
    tenantPath: ['doctor', 'appointment'],
    userPath: [],
  },
  'appointment.update': {
    roles: ['patient'],
...
const customerRoles: string[] = ['patint'];
const tenantRoles: string[] = ['clinic-owner', 'patint', 'admin'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.doctor.findFirst({
